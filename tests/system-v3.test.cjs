...
assert(app.includes('data-cats="${(p.cats||_categoriasProducto(p.name,p.cat))'), 'Cada producto debe admitir uno o más filtros correctos');
assert(index.includes('<option value="otros">'), 'Los productos nuevos sin clasificar deben seguir siendo localizables');
const categoryHelpersSource = app.slice(app.indexOf('function _categoriaPrincipalProducto'), app.indexOf('/* ── GALLERY ENGINE'));
const categoryHelpers = new Function(categoryHelpersSource + '; return {_categoriaPrincipalProducto, _categoriasProducto};')();
assert.strictEqual(categoryHelpers._categoriaPrincipalProducto('Hydroxy Max Night x120 tabs', 'otros'), 'quemador', 'Hydroxy Max Night debe aparecer en Quemadores');
...
const apiCategorySource = api.slice(api.indexOf('function inferirCat'), api.indexOf('function getStats'));
const apiCategory = new Function(apiCategorySource + '; return inferirCat;')();
assert.strictEqual(apiCategory('Codera de Compresion DRB'), 'accesorio', 'La API debe clasificar las coderas como accesorios');
...
assert(caja.includes('function _cajaCatalogoParaMostrar_'), 'La apertura debe leer STOCK_DETALLADO sin modificar hojas');
const cargaCaja = caja.slice(caja.indexOf('function obtenerDatosCajaMaxup'), caja.indexOf('function _cajaCalcular_'));
assert(!cargaCaja.includes('LockService'), 'Abrir la caja no debe esperar un bloqueo de ventas');
assert(!cargaCaja.includes('_cajaReconciliarPrincipalAntesDeVender_'), 'Abrir la caja no debe reescribir todo el stock');
assert(caja.includes('function obtenerCatalogoCajaMaxup'), 'La caja debe cargar productos por separado');
assert(caja.includes('function obtenerClientesCajaMaxup'), 'La caja debe cargar clientes por separado');
assert(caja.includes('_cajaCatalogoParaMostrar_()'), 'Abrir la caja debe mostrar el stock detallado vigente');
...
assert(cajaHtml.includes('obtenerCatalogoCajaMaxup(SESION_CAJA)'), 'La caja debe mostrar productos sin esperar a los clientes');
assert(cajaHtml.includes('obtenerClientesCajaMaxup(SESION_CAJA)'), 'La caja debe cargar clientes en paralelo');
...
assert(cajaHtml.includes("e.key==='ArrowDown'"), 'El buscador de productos debe navegar hacia abajo con el teclado');
assert(cajaHtml.includes("e.key==='ArrowUp'"), 'El buscador de productos debe navegar hacia arriba con el teclado');
...
assert(cajaWeb.includes('AbortController'), 'La carga móvil debe cortar esperas excesivas');
assert(cajaWeb.includes("location.replace(API+'?accion=caja&origen=movil')"), 'La Caja móvil debe abrir un acceso de respaldo en lugar de terminar en 404');
assert(!cajaWeb.includes("CajaMaxup.html?v='+Date.now()"), 'La interfaz móvil debe aprovechar la caché entre aperturas');
assert(cajaWeb.includes('caja_login'), 'La Caja web debe iniciar sesión con la clave administrativa');
assert(cajaWeb.includes('caja_catalogo'), 'La Caja móvil debe pedir el catálogo rápido');
assert(cajaWeb.includes('caja_clientes'), 'La Caja móvil debe pedir clientes en paralelo');
...
assert(api.includes("data.accion === 'caja_catalogo'"), 'La API debe servir el catálogo rápido a la Caja móvil');
assert(api.includes("data.accion === 'caja_clientes'"), 'La API debe servir clientes en paralelo a la Caja móvil');

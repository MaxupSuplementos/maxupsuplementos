...
test('las fichas automáticas generan textos específicos y cinco puntos editables', () => {
  ...
  assert.match(result.zma.beneficios.join(' '), /recuperación y descanso|metabolismo energético/i);
  assert.match(result.omega369.beneficios.join(' '), /función cardiovascular|cerebro y la retina/i);
  assert.match(result.biotina.beneficios.join(' '), /cabello y la piel|uñas/i);
  ...
  assert.match(result.cla.queEs + ' ' + result.cla.beneficios.join(' '), /composición corporal/i);
  assert.match(result.stanozolol.queEs, /esteroide anabólico/i);
  assert.doesNotMatch(result.stanozolol.beneficios.join(' '), /daño hepático|prohibido/i);
  ...
  Object.values(result).forEach(ficha => {
    ...
    assert.doesNotMatch(ficha.beneficios.join(' '), textoNoPromocional);
  });

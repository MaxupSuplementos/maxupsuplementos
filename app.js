...
function buildCard(p){
  ...
  const imgList = p.imgs && p.imgs.length ? p.imgs : (p.img ? [p.img] : []);
...
async function cargarDesdeSheets() {
  ...
  try {
    ...
    const sheetsProds = prods
      ...
      .map((p, i) => {
        ...
        return {
          ...
          emoji: _catEmoji(cat),
          img: img || _buscarImagen(nombre, marca),
          desc: desc,
    ...
    // ── AGRUPAR por nombre base (quitar sabor/color del nombre) ──
    const grouped = {};
    sheetsProds.forEach(p => {
      ...
      grouped[base].flavors.push({ name: saborNombre, stock: stockVal });
      // Guardar imagen específica de este sabor
      if (p.img) {
        grouped[base].imgs[saborNombre] = p.img;
        if (!grouped[base].img) grouped[base].img = p.img;
      }
    });
    
    // Reemplazar PRODUCTS con datos agrupados
    PRODUCTS.length = 0;
    Object.values(grouped).forEach(p => PRODUCTS.push(p));
...
function renderIndum() {
  ...
  for (var i = 0; i < pagina.length; i++) {
    ...
    var imgBlock = p.img ? '<div class="indum-img"><img src="'+p.img+'" alt="'+p.nombre+'" loading="lazy"></div>' : '<div class="indum-img indum-no-img" style="display:flex;align-items:center;justify-content:center;font-size:2rem">&#128085;</div>';
...
function _detectarNuevosIngresos(){
  var STOCK_KEY = 'maxup_stock_prev';
  var COLA_KEY  = 'maxup_nuevos_cola'; // cola FIFO de IDs
  var MAX_COLA  = 12;

  // Leer estado anterior
  var prevStock = {};
  var cola = []; // array ordenado: [más_antiguo, ..., más_reciente]
  try {
  ...
  PRODUCTS.forEach(function(p){
    ...
    var stockPrev = prevStock[p.id];

    if(stockPrev === undefined){
      // Producto nuevo que no existía
      nuevosDetectados.push(p.id);
    } else if(stockActual > stockPrev){
      // Stock aumentó (5→8, 0→3, cualquier aumento)
      nuevosDetectados.push(p.id);
    }
  });

  // Agregar nuevos detectados a la cola (si ya estaba, moverlo al final)
  nuevosDetectados.forEach(function(id){
    // Quitar si ya existía (para re-posicionar al final como más reciente)
    cola = cola.filter(function(x){ return x !== id; });
    cola.push(id); // agregar al final (más reciente)
  });

  // Recortar cola a máximo 12 (los más antiguos se van primero)
  ...
  // Devolver productos: más reciente primero (invertir la cola)
...
function renderNuevosIngresos(){
  ...
  // Fallback: si la cola está vacía (primera vez), mostrar primeros 8 con stock
  if(nuevos.length === 0){

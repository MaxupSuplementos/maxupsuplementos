...
const WA_DEFAULT = '5493876233406';        // WhatsApp oficial de MAXUP (sin + ni espacios)
...
function toggleAddress(){
  ...
  if(dt==='envio'){ toggleSucursal(); actualizarPesoEstimado(); }
}
...
function calcularPesoEstimado(){
  ...
  (cart||[]).forEach(function(i){
    var p = getProduct(i.pid);
    var w = (p && _PESO_CAT[p.cat]!==undefined) ? _PESO_CAT[p.cat] : 0.5;
    kg += w * (i.qty||1);
  });
  return Math.max(0.5, Math.round(kg*10)/10);
}
function actualizarPesoEstimado(){
  var el = document.getElementById('pesoEstimado');
  if(el) el.textContent = '📦 Peso estimado del pedido: ~' + calcularPesoEstimado() + ' kg';
  actualizarCotizarLink();
}
function toggleSucursal(){
  var mod = document.getElementById('envioModalidad');
  var lbl = document.getElementById('lblDireccion');
  var addr = document.getElementById('clientAddress');
  if(!mod || !lbl) return;
  if(mod.value==='sucursal'){ lbl.textContent='Sucursal del correo (nombre / dirección)'; if(addr) addr.placeholder='Ej: Sucursal Gral. Güemes Centro'; }
  else { lbl.textContent='Dirección completa (calle, número)'; if(addr) addr.placeholder='Ej: San Martín 456'; }
}
function actualizarCotizarLink(){
  var emp = (document.getElementById('envioEmpresa')||{}).value || '';
  var a = document.getElementById('cotizarLink');
  if(!a) return;
  var urls = {
    'Correo Argentino':'https://www.correoargentino.com.ar/',
    'Andreani':'https://www.andreani.com/',
    'OCA':'https://www.oca.com.ar/',
    'Vía Cargo':'https://www.viacargo.com.ar/'
  };
  if(emp==='A coordinar'){ a.style.display='none'; }
  else { a.style.display='inline-block'; a.href = urls[emp] || 'https://www.correoargentino.com.ar/'; }
}
...
function finalizarPedido(){
  ...
  const envEmpresa=document.getElementById('envioEmpresa')?.value||'';
  const envModalidad=document.getElementById('envioModalidad')?.value||'domicilio';
  const envLocalidad=document.getElementById('envioLocalidad')?.value?.trim()||'';
  const envCP=document.getElementById('envioCP')?.value?.trim()||'';
  const envPeso=(typeof calcularPesoEstimado==='function')?calcularPesoEstimado():0;
  ...
  msg+='\n🚚 *Entrega:* ' + (delivery==='envio' ? 'Envío 📦' : 'Retiro en local 🏪') + '\n';
  if(delivery==='envio') {
    msg+='🏢 *Correo:* ' + envEmpresa + (envModalidad==='sucursal' ? ' (a sucursal)' : ' (a domicilio)') + '\n';
    if(address) msg+='📍 *' + (envModalidad==='sucursal'?'Sucursal':'Dirección') + ':* ' + address + '\n';
    if(envLocalidad) msg+='📌 *Localidad:* ' + envLocalidad + (envCP ? (' (CP ' + envCP + ')') : '') + '\n';
    if(envPeso) msg+='⚖️ *Peso estimado:* ~' + envPeso + ' kg\n';
    msg+='💡 *Costo de envío a confirmar según peso/destino.*\n';
  }
  ...
  const payload = {
    ...
    email:    email,
    entrega:  delivery,
    direccion:(delivery==='envio')
      ? (envEmpresa + (envModalidad==='sucursal'?' (sucursal)':' (domicilio)') + ': ' + address
         + (envLocalidad ? (', ' + envLocalidad) : '') + (envCP ? (' CP ' + envCP) : '')
         + (envPeso ? (' ~' + envPeso + 'kg') : ''))
      : address,
    pago:     pay,
    total:    _totalFinal,
  ...
  try {
    ...
    grid.innerHTML = data.productos.map(function(p) {
      ...
      return '<div style="background:var(--dark2);border:1px solid ' + cfg.border + ';border-radius:12px;overflow:hidden">'
        ...
        + '</div>'
        + '<a href="https://wa.me/5493876233406?text=' + waMsg + '" target="_blank"'
...
function _mapCategoria(cat) {
  const m = {
    ...
    'quimicos':'quimicos','quimico':'quimicos','químicos':'quimicos','químico':'quimicos','hormonal':'quimicos','anabolico':'quimicos','anabólico':'quimicos',
...
function consultarIndum(btn) {
  ...
  window.open('https://wa.me/5493876233406?text=' + msg, '_blank');
}
...
function mostrarCombos(){
  ...
  grid.innerHTML = COMBOS.map(function(c){
    ...
    var waLink = 'https://wa.me/5493876233406?text='+waMsg;
...
function enviarStockAlert(){
  ...
  window.open('https://wa.me/5493876233406?text=' + encodeURIComponent(msg), '_blank');
  cerrarStockAlert();

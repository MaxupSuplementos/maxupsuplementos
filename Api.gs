...
function alertaVencimientosDiaria() {
  ...
  // Un mismo producto puede tener más de un lote. Para publicaciones alcanza
  // con mostrar el vencimiento más próximo, evitando flyers duplicados.
  var ofertasPorProducto = {};
  productos.forEach(function(p) {
    var claveProducto = _clavePublicacionDiaria(p.marca, p.nombre);
    if (!ofertasPorProducto[claveProducto] || p.diasRestantes < ofertasPorProducto[claveProducto].diasRestantes) {
      ofertasPorProducto[claveProducto] = p;
    }
  });
  productos = Object.keys(ofertasPorProducto).map(function(k) { return ofertasPorProducto[k]; });
  productos.sort(function(a, b) { return a.diasRestantes - b.diasRestantes; });

  var historialVencimientos = _leerHistorialPublicaciones('VENCIMIENTO');
  var publicacionesVencimiento = [];
  productos = productos.filter(function(p) {
    var descuento = p.diasRestantes < 0 ? 0 : (p.diasRestantes <= 20 ? 20 : p.diasRestantes <= 30 ? 15 : p.diasRestantes <= 50 ? 10 : 0);
    var clave = _clavePublicacionDiaria(p.marca, p.nombre);
    // Los días restantes cambian diariamente, pero el flyer sólo cambia al
    // pasar de 10% a 15% o 20%, o cuando cambia el lote/vencimiento.
    var firma = p.fechaVence + '|' + (p.diasRestantes < 0 ? 'VENCIDO-' + p.stock : (descuento ? 'OFERTA-' + descuento : 'CONTROL'));
    if (historialVencimientos[clave] && historialVencimientos[clave].firma === firma) return false;
    p._descuentoPublicacion = descuento;
    p._firmaPublicacion = firma;
    publicacionesVencimiento.push({ clave: clave, firma: firma, nombre: p.nombre });
    return true;
  });

  // Al estrenar el historial, tomar el aviso actual como punto de partida:
  // esos flyers ya fueron recibidos antes de esta mejora y no hay que volver
  // a mandarlos una vez más. Reaparecerán cuando cambie su firma/descuento.
  if (!historialVencimientos.__inicializado) {
    _guardarHistorialPublicaciones('VENCIMIENTO', publicacionesVencimiento, new Date());
    if (eliminados === 0) return { ok: true, inicializado: true, enviados: 0 };
    productos = [];
    publicacionesVencimiento = [];
  }
  ...
  if (_notificarTelegram(msg)) _guardarHistorialPublicaciones('VENCIMIENTO', publicacionesVencimiento, new Date());
}
...
var HISTORIAL_EMAILS_PROMO = '_HISTORIAL_EMAILS_PROMO';
var DIAS_ENTRE_PROMOS_IGUALES = 30;

function _normalizarClaveEmailPromo(v) {
  return String(v || '').trim().toLowerCase()
    .normalize('NFD').replace(/[\u0300-\u036f]/g, '')
    .replace(/[^a-z0-9@._+-]+/g, ' ').replace(/\s+/g, ' ').trim();
}

function _asegurarHistorialEmailsPromo() {
  var ss = _getSS();
  var hoja = ss.getSheetByName(HISTORIAL_EMAILS_PROMO);
  if (!hoja) {
    hoja = ss.insertSheet(HISTORIAL_EMAILS_PROMO);
    hoja.getRange(1, 1, 1, 5).setValues([['Fecha','Email','Telefono','Producto','Clave']]);
...
function _claveEmailPromo(email, telefono, producto) {
  var destinatario = _normalizarClaveEmailPromo(email) || String(telefono || '').replace(/\D/g, '');
  return destinatario + '|' + _normalizarClaveEmailPromo(producto);
}

function _leerPromosEnviadasRecientes(hoja, ahoraMs) {
  var recientes = {};
  if (hoja.getLastRow() < 2) return recientes;
  var limiteMs = DIAS_ENTRE_PROMOS_IGUALES * 86400000;
  hoja.getRange(2, 1, hoja.getLastRow() - 1, 5).getValues().forEach(function(r) {
    ...
    if (!isNaN(ts) && ahoraMs - ts < limiteMs && String(r[4] || '')) recientes[String(r[4])] = true;
  });
  return recientes;
}
...
function alertaOfertasClientes() {
  ...
  var zona = 'America/Argentina/Buenos_Aires';
  var lock = LockService.getScriptLock();
  if (!lock.tryLock(10000)) return { ok: true, omitido: true, motivo: 'otro_envio_en_curso' };
  try {
  var historial = _asegurarHistorialEmailsPromo();
  var recientes = _leerPromosEnviadasRecientes(historial, hoy.getTime());
  var enviados = 0, omitidosRepetidos = 0;
  var msgTelegram = '🔥 OFERTAS — Clientes para contactar\n' +
    Utilities.formatDate(hoy, zona, 'dd/MM/yyyy') + '\n\n';

  for (var i = 0; i < lista.length; i++) {
    var cli = lista[i];
    // Nunca volver a enviar el mismo producto al mismo email dentro de 30 días.
    // Esto evita que una oferta que sigue marcada como urgente genere el mismo
    // correo todos los días.
    var productosNuevos = cli.productos.filter(function(p) {
      var clave = _claveEmailPromo(cli.email, cli.telefono, p.nombre);
      if (recientes[clave]) { omitidosRepetidos++; return false; }
      return true;
    });
    if (!productosNuevos.length) continue;
    cli.productos = productosNuevos;
    ...
    if (cli.email) {
      try {
        ...
        var filasHistorial = cli.productos.map(function(p) {
          var clave = _claveEmailPromo(cli.email, cli.telefono, p.nombre);
          recientes[clave] = true;
          return [new Date(), cli.email, cli.telefono, p.nombre, clave];
        });
        historial.getRange(historial.getLastRow() + 1, 1, filasHistorial.length, 5).setValues(filasHistorial);
        enviados++;
      } catch(e) { Logger.log('Error email oferta: ' + e.message); }
    }
  }

  if (enviados) _notificarTelegram(msgTelegram);
  return { ok: true, enviados: enviados, omitidosRepetidos: omitidosRepetidos };
  } finally {
    lock.releaseLock();
  }
}
...
function _seleccionarProductosEstadosDiarios(cantidad, fecha) {
  ...
  var productos = (catalogo && catalogo.productos ? catalogo.productos : []).filter(function(p) {
    ...
    var nombreNormalizado = String(p.nombre || '').toLowerCase().trim();
    return Number(p.stock || 0) > 0 &&
      Number(p.precio_venta || p.precio || 0) > 0 &&
      String(p.imagen_url || '').trim() !== '' &&
      categoria !== 'quimicos' &&
      !/stanozol|estanozol/.test(nombreNormalizado);
  });
